...
function populateProvinceAndBairroSelects(provEl, bairroEl, defaultProv = '', defaultBairro = '') {
    if (!provEl) return;
    provEl.innerHTML = '<option value="" disabled selected>Província...</option>';
    MZ_PROVINCES_DATA.forEach(p => {
        const opt = document.createElement('option');
        opt.value = p.name;
        opt.textContent = p.name;
        if (defaultProv && p.name === defaultProv) opt.selected = true;
        provEl.appendChild(opt);
    });
    if (bairroEl && bairroEl.tagName === 'INPUT' && defaultBairro) {
        bairroEl.value = defaultBairro;
    }
}
...
async function shareProduct(productId, event) {
    ...
    const shareText = `Confira ${product.name} por apenas ${formatCurrency(product.price)} na Tchapo Tchapo!`;
...
function startQuickOrderModal(id, fromModal = false, customImg = null, customPrice = null, customTitle = null) {
    ...
    quickOrderContent.innerHTML = `
        <form id="quick-order-form" style="display: flex; flex-direction: column; flex: 1; overflow: hidden; height: 100%; width: 100%;">
                ...
                <div class="qo-right">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem;">
                            ...
                            <input type="text" id="qo-bairro" placeholder="Seu Bairro..." required style="padding: 0.65rem; border: 1.5px solid var(--gray-light); border-radius: 8px; font-size: 0.9rem; background: #fff; font-family: inherit;">
                        </div>
                        ...
                        <div class="coupon-section" style="margin-top: 0.25rem; margin-bottom: 0.25rem; width: 100%;">
            ...
            <div class="pm-sticky-bottom-bar">
                <button type="submit" id="btn-submit-quick-order" class="btn-buy-now-modal" style="width: 100%; flex: 1; font-size: 1.05rem; padding: 0.9rem; cursor: pointer;">
    ...
    populateProvinceAndBairroSelects(document.getElementById('qo-province'), document.getElementById('qo-bairro'));
...
function handleQuickOrder(e) {
    ...
    const paymentEl = document.getElementById('qo-payment');
    ...
    if (!bairro) {
        showStatusToast('⚠️ Por favor, insira o seu bairro.');
        bairroEl.focus();
        return;
    }
    if (!address) {
        showStatusToast('⚠️ Por favor, insira a sua morada / ponto de referência.');
        addressEl.focus();
        return;
    }
    if (!time) {
        showStatusToast('⚠️ Por favor, escolha um horário de entrega.');
        timeEl.focus();
        return;
    }
    if (!payment) {
        showStatusToast('⚠️ Por favor, escolha o método de pagamento.');
        paymentEl.focus();
        return;
    }
    ...
    if (devSelType !== 'none' || colorSelType !== 'none') {
        ...
        if (devSelType !== 'none') {
            if (devSelType === 'outro') {
                ...
                const selectVal = document.getElementById('qo-device') ? document.getElementById('qo-device').value : '';
                if (selectVal === 'outro') {
                    const customInput = document.getElementById('qo-custom-device');
                    const customVal = customInput ? customInput.value.trim() : '';
                    if (!customVal) {
    ...
    const submitBtn = document.getElementById('btn-submit-quick-order') || document.querySelector('.btn-checkout');
    if (submitBtn) {
...
function setupEventListeners() {
    ...
    populateProvinceAndBairroSelects(document.getElementById('customer-province'), document.getElementById('customer-bairro'));
...
function handleCheckout(e) {
    ...
    const province = provEl ? provEl.value : '';
    const bairroEl = document.getElementById('customer-bairro');
