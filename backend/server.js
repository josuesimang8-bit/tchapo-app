...
// GET /api/reviews - List all reviews or filter by product_id
app.get('/api/reviews', async (req, res) => {
    try {
        ...
        let all = Array.from(map.values()).sort((a, b) => new Date(b.created_at) - new Date(a.created_at));

        if (product_id) {
            all = all.filter(r => String(r.product_id) === String(product_id));
        }
...
app.post('/api/reviews', async (req, res) => {
    try {
        const { product_id, author_name, location, rating, comment } = req.body;

        if (!product_id) return res.status(400).json({ error: 'product_id é obrigatório.' });
        if (!author_name || !author_name.trim()) return res.status(400).json({ error: 'Nome é obrigatório.' });
        if (!rating || rating < 1 || rating > 5) return res.status(400).json({ error: 'Avaliação deve ser entre 1 e 5 estrelas.' });
        if (!comment || !comment.trim()) return res.status(400).json({ error: 'Comentário é obrigatório.' });

        const newReview = {
            id: Date.now() + Math.floor(Math.random() * 1000),
            product_id: String(product_id),
            author_name: author_name.trim(),
            location: (location || 'Moçambique').trim(),
